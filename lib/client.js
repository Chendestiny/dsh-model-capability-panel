...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function applyModelEdit(row, field, index, edit) {
			...
			if (edit.kind === "model-move") {
				/* One slot up or down. The buttons disable the boundary move, so the
				 * range check here is only belt and braces; entries travel in the
				 * array as they sit, which is what makes every hand-written field
				 * (input, reasoningEfforts, contextWindow) follow its own model. */
				const to = index + edit.delta;
				if (!Number.isInteger(to) || to < 0 || to >= models.length) return models;
		...
		function collectGroups(namespaces) {
			...
			if (pi !== undefined) {
				...
				/* Display order is the settings.yaml key order: user-layer keys first,
				 * then value-only routes in first-appearance order. The old alphabetical
				 * sort re-sorted the panel after every move, so ↑/↓ never showed what
				 * had just been written. */
				const userKeys = Object.keys(userProviders);
				const routes = [...userKeys];
				for (const route of Object.keys(valueProviders)) if (!routes.includes(route)) routes.push(route);
				const rows = [];
				for (const route of routes) {
					...
					rows.push({
						...
						/* Position inside settings.yaml's providers map: -1 when the file
						 * never declares this route — the move guard reports that instead. */
						userIndex: userKeys.indexOf(route),
		...
		const cardHeadStyle = { display: "flex", alignItems: "center", gap: "8px" };
		const iconButtonStyle = { boxSizing: "border-box", border: "1px solid var(--dsw-alias-border-l3, #d0d3d7)", borderRadius: "6px", background: "transparent", padding: "0 7px", font: "inherit", fontSize: "11px", lineHeight: "18px", color: "var(--dsw-alias-label-secondary, #4b5563)", cursor: "pointer" };
		const routeInfoStyle = { flex: "1", fontSize: "12px", lineHeight: "18px", color: "var(--dsw-alias-label-tertiary, #6b7280)" };
		const groupHeadStyle = { display: "flex", alignItems: "baseline", gap: "8px", flexWrap: "wrap" };
		...
		/**
		 * The settings path of one route's models array — what a sort write sets
		 * and what the move success line reports.
		 * @param group - the namespace group.
		 * @param row - the route row.
		 * @returns the dotted path.
		 */
		function arrayPath(group, row) {
			return group.field === "input"
				? `${group.ns}.providers.${row.route}.models`
				: `${group.ns}.models`;
		}
		...
		function successText(group, row, index, edit, entry) {
			...
			if (edit.kind === "model-move") {
				/* Report the model, where it landed, and the array it now sits in —
				 * the same real path the whole-array write set. */
				const to = index + edit.delta;
				return `已将 ${String(asRecord(entry)?.id ?? "(未命名)")} 移至第 ${to + 1} 位（${arrayPath(group, row)}）`;
			}
		...
		function CapabilitySection(props) {
			...
			/* Card folding is pure view state: no write, and the sort buttons live in
			 * the card header, so they stay reachable while a card is folded. */
			...
			/**
			 * Reorder the providers map: rebuild the user layer with this route one
			 * slot up or down, values carried over untouched, and set the whole map.
			 *
			 * The write goes through settings.mutate rather than hand-editing the file,
			 * so the revision guard and the hot reload come with it; the rebuild copies
			 * each value over as it stands, so the key order is the only thing that can
			 * change. A route the file never declares cannot be moved — that is the
			 * guard, answered in the card's own line instead of a silent no-op.
			 * @param group - the pi-ai namespace group.
			 * @param row - the route card's row.
			 * @param delta - -1 moves up, +1 moves down.
			 * @returns the write promise, or an already-settled promise when refused.
			 */
			const submitProviderMove = (group, row, delta) => {
				const rowKey = `${group.ns}|${row.route}|providers`;
				const providers = asRecord(group.userProviders);
				const keys = providers === undefined ? [] : Object.keys(providers);
				const at = keys.indexOf(row.route);
				if (keys.length === 0 || at < 0) {
					putFeedback(rowKey, { tone: "error", text: "用户层 providers 缺失，请直接改 settings.yaml 排序" });
					return Promise.resolve();
				}
				const to = at + delta;
				if (to < 0 || to >= keys.length) return Promise.resolve();
				setPending(`${rowKey}|provider-move`);
				setFailure(undefined);
				putFeedback(rowKey, { tone: "pending", text: "顺序写入中…" });
				const order = [...keys];
				order.splice(at, 1);
				order.splice(to, 0, row.route);
				const rebuilt = {};
				for (const route of order) rebuilt[route] = providers[route];
				return Promise.resolve()
					.then(() => api.settings.mutate({
						ns: group.ns,
						ops: [{ op: "set", path: ["providers"], value: rebuilt }],
						expectedRevision: group.revision
					}))
					.then((response) => {
						const view = asRecord(unwrap(response));
						...
						putFeedback(rowKey, {
							tone: "ok",
							text: `已将 ${row.route} 移至第 ${to + 1} 位（顺序热重载，无需重启）`
						});
						const timer = setTimeout(() => clearFeedback(rowKey), feedbackTtlMs);
						if (typeof timer?.unref === "function") timer.unref();
					})
			...
			const children = [
				...
				e("p", { key: "intro", style: introStyle },
					"改动即写入 settings.yaml。「输入」取消勾选即删除字段。「推理档位」打开 = 声明档位集合，关闭 = false（不提供档位），「恢复默认」删字段、交回内置目录（未声明时由目录决定是否有档位）。off 写空值，其余写档位名；已有取值不改写。"),
			...
			for (const group of groups) {
				const routeCards = group.rows.map((row) => {
					const rows = row.models.map((model, index) => {
						...
						if (group.reasoning === true) {
							...
							if (reasoning.state === "disabled") {
								controls.push(e("span", { key: `${base}|hint`, style: hintStyle }, "已禁用"));
							}
							if (reasoning.state !== "default") {
						...
						return e("div", { key: `${base}|row`, style: modelRowStyle },
							e("div", { style: modelHeadStyle },
								...
								e("button", {
									key: `${base}|move-up`, type: "button",
									"data-role": "model-move-up", "data-model": modelId, "data-route": row.route,
									style: iconButtonStyle,
									disabled: disabled || index === 0,
									onClick: () => submit(group, row, index, { kind: "model-move", delta: -1 }, "排序", entry)
								}, "↑"),
								e("button", {
									key: `${base}|move-down`, type: "button",
									"data-role": "model-move-down", "data-model": modelId, "data-route": row.route,
									style: iconButtonStyle,
									disabled: disabled || index === row.models.length - 1,
									onClick: () => submit(group, row, index, { kind: "model-move", delta: 1 }, "排序", entry)
								}, "↓")),
							...strips,
					...
					const cardKey = `${group.ns}|${row.route}`;
					const folded = collapsed[cardKey] === true;
					const cardFeedback = feedback[`${cardKey}|providers`];
					const canSort = group.orderable === true;
					return e("div", { key: `${cardKey}|card`, style: cardStyle },
						e("div", { style: cardHeadStyle },
							e("button", {
								key: `${cardKey}|fold`, type: "button",
								"data-role": "card-collapse", "data-route": row.route,
								"data-collapsed": folded ? "true" : "false",
								style: iconButtonStyle, disabled,
								onClick: () => setCollapsed((previous) => ({ ...previous, [cardKey]: !folded }))
							}, folded ? "▶" : "▼"),
							e("div", { style: routeInfoStyle },
								`${row.route === "" ? group.ns : row.route}${row.api === undefined ? "" : ` · ${row.api}`}${row.baseURL === undefined ? "" : ` · ${row.baseURL}`}${row.fromUser ? "" : " · 目录继承（首次写入会物化到用户层）"}`),
							canSort ? e("button", {
								key: `${cardKey}|move-up`, type: "button",
								"data-role": "provider-move-up", "data-route": row.route,
								style: iconButtonStyle,
								disabled: disabled || (row.userCount > 0 && row.userIndex === 0),
								onClick: () => submitProviderMove(group, row, -1)
							}, "↑") : null,
							canSort ? e("button", {
								key: `${cardKey}|move-down`, type: "button",
								"data-role": "provider-move-down", "data-route": row.route,
								style: iconButtonStyle,
								disabled: disabled || (row.userCount > 0 && row.userIndex === row.userCount - 1),
								onClick: () => submitProviderMove(group, row, 1)
							}, "↓") : null),
						cardFeedback === undefined ? null : e("p", {
							key: `${cardKey}|providers|feedback`,
							"data-feedback": cardFeedback.tone,
							style: cardFeedback.tone === "error" ? feedbackErrorStyle : feedbackOkStyle
						}, cardFeedback.text),
						...(folded ? [] : rows));
				});
				children.push(e("div", { key: `${group.ns}|group`, style: groupStyle },
					e("div", { style: groupHeadStyle },
						e("h3", { style: groupTitleStyle }, group.title),
						group.orderable === true ? null
							: e("span", { key: "order-note", style: hintStyle }, "顺序由 profile 加载顺序决定，此处不可调")),
					...routeCards));
			}
